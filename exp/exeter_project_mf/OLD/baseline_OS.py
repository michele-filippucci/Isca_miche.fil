...
exp = Experiment('exeter_project_mf_baseline_OS', codebase=cb)
...
exp.namelist = namelist = Namelist({
    ...
    'mixed_layer_nml': { 
        ...
        'land_option':'none', 
        'load_qflux':False, #no input qflux
...
if __name__=="__main__":
    ...
    run_exp = exp.derive('exeter_project_mf_baseline_OS') # derive experiment to run (this one is unchanged from specification above)
    run_exp.run(1, use_restart='$GFDL_DATA/exeter_project_mf_baseline_OS/restarts/spinup.tar.gz', num_cores=NCORES, overwrite_data=False)
    #run_exp.run(1, use_restart=False, num_cores=NCORES, overwrite_data=False)
    for i in range(2,85): # run for 16 years 
