...
exp = Experiment('exeter_project_mf_aa_OS_10_72,90', codebase=cb)
exp.inputfiles = [os.path.join(base_dir,'input/ocean_qflux_anom_10_72,90.nc')]
...
diag.add_field('dynamics', 'sphum', time_avg=False, files=['atmos_daily'])
diag.add_field('dynamics', 'omega', time_avg=False, files=['atmos_daily'])
diag.add_field('atmosphere', 'temp_2m', time_avg=False, files=['atmos_daily'])

#Tell model which diagnostics to write
diag.add_field('dynamics', 'zsurf',time_avg=True, files=['atmos_monthly']) #need at least ps, pk, bk and zsurf to do verti$
...
exp.namelist = namelist = Namelist({
    ...
    'idealized_moist_phys_nml': {
        ...
        'do_damping': False, # turns off 'damping_driver', which manages the sponge at the top 
    ...
    'mixed_layer_nml': { 
        'depth': 1., # As in O'Gorman and Schneider 
        ...
        'do_qflux': False,  # no prescribed ocean heat transport 
        'land_option':'none', 
        'load_qflux':True, #no input qflux
        'qflux_file_name':'ocean_qflux_anom_10_72,90',
    ...
    'spectral_dynamics_nml': {
        ...
        ### Taken from Tapio Schneider Github
        'surf_res': 0.01, 
        'exponent': 2., 
        'scale_heights': 5 
...
if __name__=="__main__":
    ...
    exp.run(84, use_restart='$GFDL_DATA/exeter_project_mf_baseline_OS_10_72,90/restarts/res0083.tar.gz', num_cores=NCORES, overwrite_data=False)
    #exp.run(1, use_restart=False, num_cores=NCORES, overwrite_data=False)
    #for i in range(2,84): # run for 10 years 
