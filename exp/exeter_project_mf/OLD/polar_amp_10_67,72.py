...
exp = Experiment('exeter_project_mf_aa_10_67,72', codebase=cb)
#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(base_dir,'input/ocean_qflux_anom_10_67,72.nc')]
...
diag.add_field('dynamics', 'zsurf',files=['atmos_monthly']) #need at least ps, pk, bk and zsurf to do verti$
...
diag.add_field('mixed_layer', 'flux_t', time_avg=True,files=['atmos_monthly']) #sensible heat flux at the s$
diag.add_field('mixed_layer', 'flux_lhe', time_avg=True,files=['atmos_monthly']) #latent heat flux at the s$
...
exp.namelist = namelist = Namelist({
	...
	'do_qflux':False,
	'load_qflux':True,
	'qflux_file_name':'ocean_qflux_anom_10_67,72',             
    },
...
#exp.run(46, use_restart = '$GFDL_DATA/aquaplanet_baseline/restarts/res0045.tar.gz', num_cores=NCORES)
#Lets do a run!
if __name__=="__main__":
    exp.run(84, use_restart = '$GFDL_DATA/exeter_project_mf_aa_OS_10_67/restarts/res0083.tar.gz', num_cores=NCORES)
#    exp.run(0, use_restart=False, num_cores=NCORES)
    for i in range(2,37):
