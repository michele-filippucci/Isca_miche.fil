...
exp = Experiment('exeter_project_mf_baseline_KS_ws', codebase=cb)
...
# Lets do a run!baseline_KS_ws.py
if __name__=="__main__":

    run_exp = exp.derive('exeter_project_mf_baseline_KS') # derive experiment to run (this one is unchanged$
    #run_exp.run(114, use_restart='$GFDL_DATA/exeter_project_mf_baseline_KS/restarts/res0113.tar.gz', num_cores=NCORES, overwrite_data=False)
    run_exp.run(1, use_restart=False, num_cores=NCORES, overwrite_data=True)
    for i in range(2,121): # run for 5 more years
